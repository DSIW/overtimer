...
export default function BackupReminder({ timeLogs }: Props) {
  ...
  const { isBackupNecessary, days } = useLiveQuery(() => exportImportApplicationService.getLastBackupInformation(), [], [] as Backup[]);
  ...
  if (days > 0) {
    lastBackupMessage = `Your last backup was done ${withUnit(days, "day", "days")} ago.`;
  }
