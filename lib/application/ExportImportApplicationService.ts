...
export default class ExportImportApplicationService {
  ...
  async getLastBackupInformation() {
    ...
    const days = intervalToDuration({
      start: new Date(),
      end: lastBackup.creationTime,
    }).days;
    const lastBackupOlderThanLastTimeLog =
      compareAsc(lastBackup.creationTime, lastTimeLog.startTime) === -1;

    const isBackupNecessary =
      days && days > 0 && lastBackupOlderThanLastTimeLog;
